...
public class Ability {
    ...
    public void playAnimation(String animationName, GeckoPlayer.Perspective perspective, boolean shouldLoop) {
        if (getUser() instanceof PlayerEntity && getUser().getCommandSenderWorld().isClientSide()) {
            ...
            NxAnimationController<GeckoPlayer> controller = GeckoPlayer.getAnimationController((PlayerEntity) getUser(), perspective);
    ...
    public void tick() {
        if (isUsing()) {
            if (!(getUser().getCommandSenderWorld().isClientSide) && !canContinueUsing()) AbilityHandler.INSTANCE.sendInterruptAbilityMessage(getUser(), this.abilityType);
    ...
    public void end() {
        ...
        if (getUser().getCommandSenderWorld().isClientSide) {
            heldItemMainHandVisualOverride = null;
            heldItemOffHandVisualOverride = null;
            firstPersonMainHandDisplay = HandDisplay.DEFAULT;
            firstPersonOffHandDisplay = HandDisplay.DEFAULT;
    ...
    public void codeAnimations(NxAnimatedGeoModel<? extends IAnimatable> model, float partialTick) {
    ...
    public <T extends Entity> List<T> getEntitiesNearby(LivingEntity player, Class<T> entityClass, double r) {
        return player.getCommandSenderWorld().getEntitiesOfClass(entityClass, player.getBoundingBox().inflate(r, r, r), e -> e != player && player.distanceTo(e) <= r);
    }
    ...
    public <T extends Entity> List<T> getEntitiesNearby(LivingEntity player, Class<T> entityClass, double dX, double dY, double dZ, double r) {
        return player.getCommandSenderWorld().getEntitiesOfClass(entityClass, player.getBoundingBox().inflate(dX, dY, dZ), e -> e != player && player.distanceTo(e) <= r);
    }
